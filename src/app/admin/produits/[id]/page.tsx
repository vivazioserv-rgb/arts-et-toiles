...
export default function ModifierProduitPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/products/${id}`, { cache: "no-store" })
      ...
      .then((p) => {
        ...
        setDelay(String(p.delay || "48"));
        setIsNew(!!p.isNew);
        setStatus(p.status || "available");
        setCategory(p.category || "");
        setImageUrl(p.imageUrl || "");
        setFlavors(p.flavors?.length ? p.flavors : [{ name: "Option standard", surcharge: 0 }]);
        setSizes(p.sizes?.length ? p.sizes : [{ name: "Format standard", surcharge: 0 }]);
        setLoading(false);
      })
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert("Erreur lors de l'upload. Utilise une URL externe.");
      }
    } catch {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <h1 className="mb-8 font-serif text-3xl">Modifier : {name}</h1>
            ...
            {flavors.map((f, i) => (
              <div key={i} className="flex items-center gap-2">
                <input value={f.name} onChange={(e) => updateFlavor(i, "name", e.target.value)}
                  placeholder="Nom" className="flex-1 rounded-lg border border-gray-300 px-3 py-2 text-sm" />
