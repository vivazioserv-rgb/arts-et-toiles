...
function reducer(state: CartItem[], a: Action): CartItem[] {
  switch (a.type) {
    ...
    case "REMOVE": return state.filter((i) => i.key !== a.key);
    case "QTY": return state.map((i) => (i.key === a.key ? { ...i, quantity: Math.max(1, a.qty) } : i));
    case "CLEAR": return [];
  }
}
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo<Ctx>(
    () => ({
      ...
      addToCart: (item) => { dispatch({ type: "ADD", item }); setCartOpen(true); },
