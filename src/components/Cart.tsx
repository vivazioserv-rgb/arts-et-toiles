    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <aside className="flex h-full w-full max-w-md flex-col bg-[var(--muted)] shadow-2xl">
        <div className="flex items-center justify-between border-b border-[var(--accent)] p-5">
          ...
          <button onClick={() => setCartOpen(false)} aria-label="Fermer"><X className="h-5 w-5" /></button>
        </div>
                  ...
                  <div className="flex flex-1 flex-col justify-between">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <button onClick={() => updateQty(item.key, item.quantity - 1)} className="rounded-full bg-[var(--muted)] p-1" aria-label="Moins">
                          <Minus className="h-3 w-3" />
                        </button>
                        <span className="w-6 text-center text-sm">{item.quantity}</span>
                        <button onClick={() => updateQty(item.key, item.quantity + 1)} className="rounded-full bg-[var(--muted)] p-1" aria-label="Plus">
        ...
        {cart.length > 0 && (
          <div className="border-t border-[var(--accent)] p-5">
            ...
            <button
              onClick={() => { setCartOpen(false); router.push("/commander"); }}
